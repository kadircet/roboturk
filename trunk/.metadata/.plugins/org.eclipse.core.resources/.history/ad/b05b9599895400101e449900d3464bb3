...
//  ssl_msgs::GlobalRobotState robot_state;
  ...
  total_path_distance+=getDistance(robot_state.pose,prev_robot_state.pose);
  prev_robot_state = robot_state;
  dist_to_goal = getDistance(robot_state.pose,target_pose);
  if(dist_to_goal < 0.2)
    std::cout<<"distance: "<<total_path_distance<<std::endl;
