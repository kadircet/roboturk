...
struct Arrow{
	...
	Arrow(float force_vector[3],float robot_position[3]){
		offset[0]=robot_position[0];//x
		offset[1]=robot_position[1];//y
		offset[2]=robot_position[2];//z

		//length of the force vector
		length= sqrt( 	pow(force_vector[0], 2.0)+
				pow(force_vector[1], 2.0)+
				pow(force_vector[2], 2.0));
		//direction of the force vector w.r.t global frame
		angle = atan2( 	force_vector[0],force_vector[2]);
